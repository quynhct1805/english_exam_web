    ...
    <div class="main px-3">
      ...
      <div class="content">
        ...
        <v-expansion-panels variant="popout" class="panel-part my-4">
          <v-expansion-panel v-for="(part, indexPart) in parts" :key="part.id" class="question-info">
            <v-expansion-panel-title expand-icon="mdi-menu-down">
              <strong>{{ part.name }}</strong>
              <v-spacer></v-spacer>
              <v-btn variant="text" color="error" icon="mdi-delete" size="small" style="height: 18px; width: 18px"
                @click="deletePart(part.id, indexPart)"></v-btn>
            </v-expansion-panel-title>
            ...
            <v-expansion-panel-text>
                  ...
                  <v-row>
                    ...
                    <v-col>
                      <div class="question-text">
                        ...
                        <v-btn variant="text" color="error" icon="mdi-delete" size="small"
              ...
              <v-form class="add-question" v-if="addQues" v-click-outside="{
                ...
                <v-row no-gutters>
                  <v-col cols="2">
                    <v-text-field v-model="newQues.name" variant="outlined" label="Tên" density="compact"
                  ...
                  <v-col cols="8" class="mr-4 mb-3">
                    ...
                    <v-btn variant="outlined" color="#4a7c59" @click="addAnswer()">
                      Thêm đáp án
                    </v-btn>
                  </v-col>
                  <v-col cols="3">
                    <v-text-field v-model="newQues.true_answer" variant="outlined" label="Đáp án đúng"
                ...
                <v-btn color="success" text @click="saveNewQues(indexPart)" type="submit">
                  Lưu
                </v-btn>
              </v-form>
            </v-expansion-panel-text>
        ...
        <v-btn class="add-btn included mb-2" variant="outlined" color="#4a7c59" prepend-icon="mdi-plus"
          @click="addPart = true">Thêm bài
        </v-btn>
          ...
          <v-form class="add-part" v-click-outside="{
            ...
            <v-row no-gutters>
              <v-col cols="2" class="mr-4">
                <v-text-field v-model="newPart.name" variant="outlined" label="Tên *" density="compact"
                  :rules="textRules"></v-text-field>
              </v-col>
              <v-col cols="2" class="mr-4">
                <v-text-field type="number" v-model="newPart.total_ques" variant="outlined" label="Tổng số câu hỏi *"
                  density="compact" :rules="numberRules"></v-text-field>
              </v-col>
              <v-col cols="3" class="mr-4">
                <v-text-field v-model="newPart.audio" variant="outlined" label="Tên file nghe"
                  density="compact"></v-text-field>
              </v-col>
              <v-col cols="12" class="mt-3">
                <v-textarea v-model="newPart.paragraph" variant="outlined" label="Đoạn văn" density="compact" rows="2"
                  messages="** Thêm tên file nghe, đoạn văn nếu có"></v-textarea>
              </v-col>
              <v-col cols="12" class="mt-3">
                <v-textarea v-model="newPart.description" variant="outlined" label="Mô tả" density="compact"
                  rows="2"></v-textarea>
              </v-col>
            </v-row>
...
const listAns = ref(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K'])
const addAnswer = () => {
  newQues.value.answers.push(listAns.value[newQues.value.answers.length] + ". ");
};
...
const removeAnswer = (index) => {
  ...
  for (const [idx, answer] of newQues.value.answers.entries()) {
    newQues.value.answers[idx] = answer.replaceAt(0, listAns.value[idx])
  }
};
