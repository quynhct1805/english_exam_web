<template>
  ...
  <v-layout>
        ...
        <v-table class="manage-user" :key="users.length">
          ...
          <v-row class="content" v-for="(user, index) in users" :key="user.id" :class="{ background: index % 2 == 0 }">
            ...
            <v-col class="text-center" cols="1">
              ...
              <v-btn color="error" icon="mdi-delete" size="small" variant="text" @click="deleteUser(user.id)"></v-btn>
            </v-col>
            <v-divider></v-divider>
          </v-row>
        </v-table>
    ...
    <v-dialog v-model="openDialog" persistent>
      <UserForm class="form" :action="action" :user-info="userInfo" @changeUser="(res) => (openDialog = res)"
